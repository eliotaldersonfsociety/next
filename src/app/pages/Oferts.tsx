...
export default function HomePage() {
  ...
  const scroll = useCallback((direction: "left" | "right") => {
    if (scrollContainerRef.current) {
      ...
      if (direction === "left") {
        ...
        container.scrollBy({ left: scrollAmount, behavior: "smooth" })
        setCurrentProductIndex(Math.min(products.length - 1, currentProductIndex + 1))
      }
    }
  ...
  return (
    <div className="w-full max-w-7xl bgmx-auto px-4 py-8">
      ...
      <div className="relative">
        ...
        >
          ...
          <Card className="min-w-[280px] max-w-[280px] h-[400px] flex flex-col justify-center items-center snap-start bg-gradient-to-br from-primary/80 to-primary shadow-lg border-0">
            <CardContent className="flex flex-col items-center justify-center text-center p-6">
              <h3 className="text-3xl font-bold text-primary-foreground mb-4">Ofertas del Día</h3>
          ...
          {products.map((product) => (
            ...
            >
              <CardContent className="p-4">
                ...
                <div className="mt-2">
                  <span className="text-xl font-bold text-primary">${product.price || product.sale_price}</span>
        ...
        <div className="flex justify-center gap-1 mt-4 md:hidden">
          {products.map((_, i) => (
            <div
              key={i}
              className={`h-1.5 rounded-full transition-all ${i === currentProductIndex ? "w-6 bg-primary" : "w-1.5 bg-muted"}`}
            />
          ))}
        </div>
      </div>
